let h3 = document.querySelector('h3');
let color1 = document.querySelector('.color1');
let color2 = document.querySelector('.color2');
let body = document.querySelector('#gradient');
let angleInput = document.querySelector("#angle-input")



function backgroundChange() {
    body.style.background = `linear-gradient(${storeAngleInput()}deg, ${color1.value},
        ${color2.value})
        `;
    h3.textContent = body.style.background;

}
function storeAngleInput() {
    return angleInput.value
}


angleInput.addEventListener('change', storeAngleInput)
color1.addEventListener('input', backgroundChange);
color2.addEventListener('input', backgroundChange)

window.onLoad = setInterval(backgroundChange, 500)
